...
public class SwitchBot implements RoShamBot {
    ...
    public Action getNextMove(Action lastOpponentMove) {
        Action historyAction = getNextMoveHistory(lastOpponentMove);
        Action markovAction = getNextMoveMarkov(lastOpponentMove);
        if (this.oppPlays.size() > this.turns) {
            Random rand = new Random();
            int rand1 = 0;
            if (this.markov) {
                rand1 = rand.nextInt(500, 2000) + 1;
            } else {
                rand1 = rand.nextInt(0, 500) + 1;
            }
            this.turns += rand1;
            this.markov = !this.markov;
        }
        if (this.markov) {
            return markovAction;
        }
     ...
     */
    public Action getNextMoveHistory(Action lastOpponentMove) {
        checkIfLosing();
        this.bestPlay = getRandomMove();
    ...
    private void generateKeys(int remaining, List<Action> currentKey,
            List<List<Action>> keys) {
        if (remaining == 0) {
    ...
    public Action predict() {
        ...
        if (probs.values().stream().map(Observation::getProb).max(Double::compareTo)
            ...
            return probs.entrySet().stream()
                    .max(Map.Entry.comparingByValue((o1, o2) -> Double.compare(o1.prob,
                            o2.prob)))
    ...
    public Action getNextMoveMarkov(Action lastOpponentMove) {
